...
__global__ void downsweep(int *output, int two_d, int N) {
  if (two_d == N / 2) {
    output[N - 1] = 0;
  }
  int index = blockIdx.x * blockDim.x + threadIdx.x;
...
void exclusive_scan(int *input, int N, int *result) {
  ...
  for (int two_d = N2 / 2; two_d >= 1; two_d /= 2) {
    ...
    int numThreads = N2 / two_dplus1;
    int blocks = (numThreads + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK;
    downsweep<<<blocks, THREADS_PER_BLOCK>>>(result, two_d, N2);
    cudaDeviceSynchronize();
  }
}
...
int find_repeats(int *device_input, int length, int *device_output) {
  ...
  isRepeat<<<blocks, THREADS_PER_BLOCK>>>(device_input, length, device_output);
  cudaDeviceSynchronize();

  exclusive_scan(device_input, length, device_output);
  int *output = new int(0);

  cudaMemcpy(output, device_output + length - 1, sizeof(int),
             cudaMemcpyDeviceToHost);
