...
#pragma omp for schedule(dynamic, 200)
    ...
    memcpy(new_frontier->vertices + start_idx, local_frontier,
...
void bottom_up_step(Graph g, vertex_set *frontier, vertex_set *new_frontier,
                    int *distances, const int frontier_dist) {
...
#pragma omp for schedule(dynamic, 200)
    for (int i = 0; i < g->num_nodes; i++) {
      ...
      for (int neighbor = start_edge; neighbor < end_edge; neighbor++) {
        ...
        if (distances[incoming] == frontier_dist) {
          distances[i] = frontier_dist + 1;
